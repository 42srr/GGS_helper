...
async function createMultipleAdmins() {
  ...
  for (const adminData of adminAccounts) {
    try {
      const existingAdmin = await userService.findByUsername(adminData.username);
...
async function promoteUserToAdmin() {
  ...
  // 명령행 인수에서 사용자 username 가져오기
  const username = process.argv[3];

  if (!username) {
    console.log('❌ Usage: npm run create-admin promote <username>');
    await app.close();
    return;
  }
...
async function main() {
  ...
  switch (command) {
    ...
    default:
      ...
      console.log(
        '  npm run create-admin promote <username>  # Promote existing user to admin',
      );
      console.log('  npm run create-admin list         # List all users and roles');
      console.log('');
      console.log('Examples:');
      console.log('  npm run create-admin single');
      console.log('  npm run create-admin promote john_doe');
